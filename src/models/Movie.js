...
const Movie = {
  ...
  async findByIdAndUpdate(
    ...
    if (actors !== undefined) {
      // Delete existing actor relations
      await db("movie_actors")
        .where({ movie_id: id })
        .del();

      // Insert new actor relations
      if (actors.length > 0) {
        const actorRelations =
          actors.map((actorId) => ({
            movie_id: id,
